...
class KdTree(object):
    def __init__(self, tree_type, pointsList, nclose=0):
        self.nclose = nclose
        self.tree_type = tree_type
        if tree_type not in ['node', 'element']:
            # verifies you're calling the right
    ...
    def getCloseElementIDs(self, point):
        close_nodes_dists = self.nNearestPoints(point, self.nclose)
        close_ids = []
        dists = []
    ...
    def PointsInSphere(self, tree, p, radius, depth=0, ptlist=None):
        ...
        if tree.left_child:
            if tree.location[axis] >= p[axis] - radius:
                self.PointsInSphere(tree.left_child, p, radius, depth + 1, ptlist)
        if tree.right_child:
            if tree.location[axis] <= p[axis] + radius:
                self.PointsInSphere(tree.right_child, p, radius, depth + 1, ptlist)
        return ptlist
